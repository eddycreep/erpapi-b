...
//listener for the 'get-customer-basket-items' event
        ...
        Basket.getCustomerBasketItems(basketId, (err, basket_items) => {
            if (err) {
                ...
                if (basket_items && basket_items.length > 0) {
                    // Loop through all basket items and log the details
                    basket_items.forEach(item => {
                        const { product, quantity, product_price } = item;

                        console.log(`Customers Purchased Product "${product}", the quantity "${quantity}", the price "${product_price}"`);
                        
                        // Process each basket item (e.g., emit event or save to DB)
                        // You can use any necessary logic here for each item
                        event.emit('get-product-details', product) //customer id retrieved from get total basket
                    });

                    // Emit event after processing all basket items (or after relevant actions)
                    event.emit('check-loyalty-customer', customerid); // Assuming customerid is available
                } else {
                    console.log('There was no basket information returned from result.');
                }
            }
...
//listener for the 'get-product-details' event
